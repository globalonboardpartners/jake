...
#[post("/event")]
async fn create_event(req: HttpRequest, event: Json<Event>) -> HttpResponse {
...
#[get("/event")]
async fn get_event_by_id_or_all(req: HttpRequest, Query(id): Query<Id>) -> HttpResponse {
    if id.id.is_some() {
        match db::connect(req).await {
            Ok(pg) => {
                let returned: Result<Event, Error> = sqlx::query_as!(
...
#[put("/event")]
async fn update_event(req: HttpRequest, event: Json<Event>) -> HttpResponse {
    match db::connect(req).await {
        Ok(pg) => {
            let returned: Result<Event, Error> = sqlx::query_as!(
                Event,
...
#[delete("/event")]
async fn delete_event(req: HttpRequest, id: Json<Id>) -> HttpResponse {
    match db::connect(req).await {
        Ok(pg) => {
