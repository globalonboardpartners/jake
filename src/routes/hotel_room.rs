...
#[post("/hotel_room")]
async fn create_hotel_room(hotel_room: Json<HotelRoom>) -> HttpResponse {
...
#[get("/hotel_room")]
async fn get_hotel_room_by_id_or_all(Query(id): Query<Id>) -> HttpResponse {
    if id.id.is_some() {
        match db::connect().await {
            Ok(pg) => {
                let returned: Result<HotelRoom, Error> = sqlx::query_as!(
...
#[put("/hotel_room")]
async fn update_hotel_room(hotel_room: Json<HotelRoom>) -> HttpResponse {
    match db::connect().await {
        Ok(pg) => {
            let returned: Result<HotelRoom, Error> = sqlx::query_as!(
                HotelRoom,
...
#[delete("/hotel_room")]
async fn delete_hotel_room(id: Json<Id>) -> HttpResponse {
    match db::connect().await {
        Ok(pg) => {
