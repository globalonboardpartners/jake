...
#[post("/activity")]
async fn create_activity(req: HttpRequest, activity: Json<Activity>) -> HttpResponse {
...
#[get("/activity")]
async fn get_activity_by_id_or_all(req: HttpRequest, Query(id): Query<Id>) -> HttpResponse {
    if id.id.is_some() {
        match db::connect(req).await {
            Ok(pg) => {
                let returned: Result<Activity, Error> = sqlx::query_as!(
...
#[put("/activity")]
async fn update_activity(req: HttpRequest, activity: Json<Activity>) -> HttpResponse {
    match db::connect(req).await {
        Ok(pg) => {
            let returned: Result<Activity, Error> = sqlx::query_as!(
                Activity,
...
#[delete("/activity")]
async fn delete_activity(req: HttpRequest, id: Json<Id>) -> HttpResponse {
    match db::connect(req).await {
        Ok(pg) => {
