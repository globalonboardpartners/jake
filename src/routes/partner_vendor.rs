...
#[post("/partner_vendor")]
async fn create_partner_vendor(req: HttpRequest, partner_vendor: Json<PartnerVendor>) -> HttpResponse {
...
#[get("/partner_vendor")]
async fn get_partner_vendor_by_id_or_all(req: HttpRequest, Query(id): Query<Id>) -> HttpResponse {
    if id.id.is_some() {
        match db::connect(req).await {
            Ok(pg) => {
                let returned: Result<PartnerVendor, Error> = sqlx::query_as!(
...
#[put("/partner_vendor")]
async fn update_partner_vendor(req: HttpRequest, partner_vendor: Json<PartnerVendor>) -> HttpResponse {
    match db::connect(req).await {
        Ok(pg) => {
            let returned: Result<PartnerVendor, Error> = sqlx::query_as!(
                PartnerVendor,
...
async fn delete_partner_vendor(req: HttpRequest, id: Json<Id>) -> HttpResponse {
    match db::connect(req).await {
        Ok(pg) => {
